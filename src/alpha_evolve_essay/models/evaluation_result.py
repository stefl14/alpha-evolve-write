...
class EvaluationResult(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Unique identifier for the evaluation"
    )
    ...
    criteria_scores: dict[str, float] = Field(
        default_factory=dict, description="Scores for individual evaluation criteria"
    )
    feedback: str = Field(default="", description="Textual feedback about the essay")
    evaluator: str = Field(
        default="unknown", description="Name or ID of the evaluator (human/LLM)"
    )
    ...
    def validate_criteria_scores_range(cls, v: dict[str, float]) -> dict[str, float]:
        ...
        """
        for _criterion, score in v.items():
